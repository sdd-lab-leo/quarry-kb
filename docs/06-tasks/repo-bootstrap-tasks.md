...
- OQ-09 values are not needed for bootstrap-only tests.
...
- Internal-only embedding/OCR host policy.
...
- Vue shell, typed health client, and status states.
...
- **Scope:** Compose file, service environment wiring, local volume boundary, network dependency order, and non-secret `.env.example`.
- **Dependencies:** None.
- **Owner type:** platform / devops
- **Priority:** Must
- **Notes:** Do not add real secrets, real documents, or a live gateway requirement. Traceability: `REQ-BOOT-001`, `REQ-BOOT-005`, `REQ-BOOT-008`.
- **Definition of done:** Compose configuration validates; services have documented health behavior; `.env.example` contains placeholders only.
...
- **Scope:** PostgreSQL/pgvector service choice, Alembic configuration, baseline migration, vector capability validation, and migration command documentation.
- **Dependencies:** None.
- **Owner type:** backend / platform
- **Priority:** Must
- **Notes:** Do not create user, document, chunk, session, message, citation, or audit tables. Traceability: `REQ-BOOT-002`, `REQ-BOOT-003`, `REQ-BOOT-007`.
- **Definition of done:** Fresh database migration succeeds; reapplication is safe; missing vector capability is surfaced.

### TASK-003: Implement API Settings And Health Endpoints

- **Objective:** Expose truthful liveness and readiness using the API guide.
- **Scope:** Typed settings loading, embedding/OCR allowlist validation, safe redaction, response envelope, `/api/v1/health/live`, and `/api/v1/health/ready`.
...
- **Definition of done:** Endpoint behavior passes ready, database-down, migration-pending, vector-missing, and invalid-config cases without leaking secrets.
...
- **Scope:** Vue application shell, single typed API client, ready/not-ready/error/loading states, and plain CSS variables.
...
- **Definition of done:** Production build succeeds; shell can display API success and safe failure states.

### TASK-005: Add Foundation Verification Loop

- **Objective:** Prove the vertical bootstrap path from Compose to browser health display.
- **Scope:** Backend tests, configuration policy tests, API integration tests, migration upgrade/reapply smoke, frontend build, Compose validation, and frontend-to-backend health smoke.
- **Dependencies:** TASK-001, TASK-002, TASK-003, TASK-004.
- **Owner type:** QA / backend / frontend / devops
- **Priority:** Must
- **Notes:** Tests use fake/local dependencies and mock configuration; no paid model or real company data. Traceability: `REQ-BOOT-009`, all acceptance criteria.
