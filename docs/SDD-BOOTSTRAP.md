...
- Language: English-only for SDD and project rules.
...
Note: the skill directory name `wwa-sdd-generate-all` is kept for isomorphism with the shared Agentic SDLC skill set. Apply it to Quarry KB slices using this bootstrap and `docs/00-context/sdd-profile.md`.
...
Filename note: keep historical names (`-requirement`, `-user-stories`) for chain isomorphism.
...
- [ ] Auth, roles, upload/knowledge, and retrieval boundaries are explicit when relevant
- [ ] Security / audit constraints are explicit when relevant
