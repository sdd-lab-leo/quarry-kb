...
- ADR-0006 is proposed for pilot auth security defaults (`auth_version` mandatory, Argon2id, HS256, bootstrap env vars, browser storage, last-Admin protection). Owner/security acceptance is required before TASK-AUTH-008 completes.

## Handoff Gate

Implementation is not approved by this document set alone. Before a coding session:

1. Resolve OQ-AUTH-001 through OQ-AUTH-005 by accepting or amending ADR-0006.
