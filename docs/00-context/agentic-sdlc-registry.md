...
| Tool | Path | Status |
|---|---|---|
| Codex | `~/.codex/skills/` | Active (when synced) |
| Shared OpenAI-style agents | `~/.agents/skills/` | Active (when synced) |
| Claude Code | `~/.claude/skills/` | Active (when synced) |
| OpenCode | `~/.config/opencode/skills/` | Active (when synced) |
| OpenCode command router | `~/.config/opencode/commands/sdd.md` | Active (when synced) |
| GitHub Copilot repository bridge | `.github/copilot-instructions.md` | Deferred (not in Step 1) |
| GitHub Copilot Agentic SDLC instructions | `.github/instructions/agentic-sdlc.instructions.md` | Deferred (not in Step 1) |
...
| GitHub Copilot repository instructions | `.github/copilot-instructions.md` | Repository-wide Copilot bridge into Agentic SDLC | Deferred |
| GitHub Copilot Agentic SDLC instructions | `.github/instructions/agentic-sdlc.instructions.md` | Copilot path-specific and agent routing for shared skills | Deferred |
| Agentic SDLC CI | `.github/workflows/agentic-sdlc.yml` | CI template for doctor, secret scan, backend test, and frontend build | Deferred |

## Maintenance Rules

- Update this registry whenever a global skill or supporting asset is added, renamed, removed, or materially changed.
- Run `scripts/sync-global-agent-assets.sh` after changing canonical skills once that script exists.
- Run `scripts/agentic-sdlc-doctor.sh` after sync and before relying on global tool behavior once that script exists.
