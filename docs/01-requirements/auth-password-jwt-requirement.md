...
| AC-AUTH-010 | Bootstrap env vars create exactly one Admin when none exist and are ignored once any Admin exists; missing/invalid required bootstrap values with zero Admins fail closed at startup and create no Admin. |
...
| OQ-AUTH-003 | How long should access JWTs live and how should expiry be represented? | 30-minute access token; required `sub`/`iat`/`exp`/`auth_version`; UTC NumericDate claims; response `expires_at` mirrors `exp`; no refresh token; no `role` claim in pilot tokens. | Session UX and security posture |
